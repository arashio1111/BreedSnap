...
const LoadScreen = ({route}) => {
  ...
  const fetchData = async () => {
    try {
      const Selectscreen = await RNFS.readFile(`${RNFS.DocumentDirectoryPath}/selected.txt`, 'utf8');
      ...
      const selectedBreed = sortedPredictions[0][0]; // 这应该是预测出的品种
      ...
      navigation.navigate('ResultScreen', { 
        ...
        selectedBreed: selectedBreed, // 传递预测的品种给 SaveInputScreen 组件
      });
    } catch (error) {
